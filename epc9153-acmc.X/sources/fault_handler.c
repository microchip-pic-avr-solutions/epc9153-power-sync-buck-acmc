 ...
 *    When no second  value is used, set ref_obj = NULL (NULL-pointer).
 * 
...
volatile uint16_t fault_check(volatile FAULT_OBJECT_t* fltobj) {
    ...
    // If the source object is not initialized, exit here
...
volatile uint16_t ocp_trip_response(void)
{
    // This user-defined function will be called when a fault condition has been tripped
...
volatile uint16_t regulation_trip_response(void)
{
    // This user-defined function will be called when a fault condition has been tripped
    while(PG1STATbits.UPDATE);
    //disable PWM
...
volatile uint16_t ocp_reset_response(void)
{
