...
//                U1TXREG = 'C';
//                U1TXREG = 'U'; 
            uint16_t adc2 = temperature_adc; //reading from ADCBUF to unsigned int  Current1
    ...
    if(U1STAbits.TRMT == 1) { // TX buffer empty, we can write
        ...
        if (tx_count == 1)
        {
            uint16_t adc4 = cs_calib_offset;
            uint16_t adc5 = counter_pd_max;
            uint16_t adc6 = max_iout;//ADCMP3CONbits.STAT;
