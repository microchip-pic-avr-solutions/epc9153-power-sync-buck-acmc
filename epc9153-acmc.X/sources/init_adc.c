...
volatile uint16_t ConfigureADC(void) {
    ...
    ADCON2Hbits.SHRSAMC = 10; // Shared ADC Core Sample Time Selection: 10x TADs sampling time
    ...
    // ADC CMP for short circuit
    ADCMP0ENLbits.CMPEN1 = 1;
    ADCMP0CON = 0xC2; // LOHI enabled; CMPEN enabled; IE enabled; Generates a digital comparator event when ADCBUFx >= ADCMPnLO
    ADCMP0LO = IOUTADC(ILIMITSCP)+ISENSEREF;
    
    // ADC CMP for zero load
//    ADCMP3ENLbits.CMPEN1 = 1;
//    ADCMP3CON = 0x81; // LOLO enabled; CMPEN enabled; IE disabled; Generates a digital comparator event when ADCBUFx >= ADCMPnLO
//    ADCMP3LO = 2077;//IOUTADC(0.25)+ISENSEREF;

    _TRGSRC0 = 0b00100;  // PWM 1 trigger 1
    _TRGSRC1 = 0b00100;  // 
...
//    _ADCAN0IE = 1;
    ...
    ADEIELbits.EIEN1 = 1; // Early interrupt is enabled for the channel
    ADIELbits.IE1 = 1;
    _ADCAN1IP = 5;
    _ADCAN1IF = 0;
    _ADCAN1IE = 1;    
    
    _ADCMP0IP = 6; // higher level interrupt gets priority
    _ADCMP0IF = 0;
    _ADCMP0IE = 1;
//    
////    ADIELbits.IE3 = 1;
