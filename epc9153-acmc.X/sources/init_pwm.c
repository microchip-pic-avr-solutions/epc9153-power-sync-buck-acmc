...
//    PG1CONL = 0x0088;  // High-Resolution enabled, PWM Generator uses Master clock selected by the MCLKSEL[1:0] (PCLKCON[1:0]) control bits, Edge PWM2H
    PG1CONL = 0x0008;   // High-Resolution disabled
    ...
    PG1EVTLbits.ADTR1EN2    = 0b1;          // PG1TRIGB  Compare Event is disabled as trigger source for ADC Trigger 1
    PG1EVTLbits.ADTR1EN1    = 0b0;          // PG1TRIGA  Compare Event is  enabled as trigger source for ADC Trigger 1
    PG1EVTLbits.UPDTRG      = 0b01;         // PG1DC triggers update 
    ...
    PG1EVTHbits.IEVTSEL     = 0b01;//11;         // Time base interrupts are disabled
    ...
    PG1EVTHbits.ADTR2EN1    = 0b1;          // PG1TRIGA register compare event is disabled as trigger source for ADC Trigger 2
...
//    PG1FFPCIL       = 0x0000;          // PWM GENERATOR FF PCI REGISTER LOW  (not used)
    ...
    // FF PCI setup
    PG1FFPCILbits.TSYNCDIS = 0; // Termination of latched PCI occurs at PWM EOC
    PG1FFPCILbits.TERM = 0b001; // Auto-Terminate: Terminate when PCI source transitions from active to inactive
    PG1FFPCILbits.AQPS = 1;     // Inverted
    PG1FFPCILbits.AQSS = 0b010; // LEB is active
    PG1FFPCILbits.PSS = 0b11011;// Comparator 1 output
    
    PG1FFPCIHbits.ACP = 0b011; // Latched
    PG1FFPCIHbits.BPSEL = 0b001; // PG2
    PG1FFPCIHbits.BPEN = 1; // not bypassed
...
//    PG1LEBL     = 0x0000;   // Leading Edge Blanking not used
    PG1LEBHbits.PHR = 1;     // Rising edge of PWMxH will trigger the LEB duration counter
    PG1LEBL     = TON_PWM1;   // Leading Edge Blanking time
    ...
    PG1TRIGA    = (PWM_PERIOD >> 1) + (PWM_PERIOD >> 2); //(DUTY_FIXED >> 1);

    // PGxTRIGB: PWM GENERATOR x TRIGGER B REGISTER
    PG1TRIGB    = (PWM_PERIOD >> 1);
...
//    while((PG1STATbits.UPDATE)  && (timeout++<8000));
