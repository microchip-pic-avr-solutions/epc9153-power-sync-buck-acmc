...
volatile uint16_t ConfigureOscillator(void) {
    ...
    if(OSCCONbits.COSC != 0b000)
    {
        ...
        while((OSCCONbits.COSC != OSCCONbits.NOSC) && (timeout++ < 8000));
        if (timeout >= 8000){ return(0); }
    }
    ...
    PLLFBDbits.PLLFBDIV = 70; // FVCO = 70 * 8 = 560 MHz
    PLLDIVbits.POST1DIV = 2; // N2=2  FPllO = 560 / 2 = 280 MHz. FPLLO/2 goes to FOSC (140 MHz), CPU clock is FOSC/2
    ...
    if(OSCCONbits.COSC != OSCCONbits.NOSC)
    {
        ...
        while((OSCCONbits.COSC != OSCCONbits.NOSC) && (timeout++ < 8000));
        if ((OSCCONbits.COSC != OSCCONbits.NOSC) || (timeout >= 8000))
        { return(0); }
    }
    ...
    while((OSCCONbits.LOCK != 1) && (timeout++ < 8000)); // Wait n while loops for PLL to Lock
	if ((OSCCONbits.LOCK != 1) || (timeout >= 8000)) // Error occurred?
	...
	APLLFBD1bits.APLLFBDIV = 62;   // M  = APLLFBD
    APLLDIV1bits.APOST1DIV = 1;     // N2 (non zero)
...
//    // Add enforced delay
    ...
    while((ACLKCON1bits.APLLCK != 1) && (timeout++<10000));
	if ((ACLKCON1bits.APLLCK != 1) || (timeout++ >= 10000))	// PLL still not locked in?
	{ return (0); } // => If so, return error code
    else
